...
#define SOC_ARM32_NEON_FORCE_INLINE inline __attribute__((always_inline))
#else
#define SOC_ARM32_NEON_FORCE_INLINE inline
#endif

static size_t merge_depth_planes_f32_neon_vectorized_arm32(
    float* level_zero,
...
)
{
    ...
    while (element_count - element_index >= 16u) {
        ...
        for (plane_index = 0u;
            ...
            merged0 = vbslq_f32(
                vcgtq_f32(candidate0, merged0),
                candidate0,
                merged0
            );
            merged1 = vbslq_f32(
                vcgtq_f32(candidate1, merged1),
                candidate1,
                merged1
            );
            merged2 = vbslq_f32(
                vcgtq_f32(candidate2, merged2),
                candidate2,
                merged2
            );
            merged3 = vbslq_f32(
                vcgtq_f32(candidate3, merged3),
                candidate3,
                merged3
            );
...
static void merge_depth_planes_f32_neon_arm32(
    float* level_zero,
    const float* scratch_planes,
    size_t element_count,
    size_t scratch_plane_stride,
...
)
{
    ...
    merged_element_count = merge_depth_planes_f32_neon_vectorized_arm32(
        level_zero,
        scratch_planes,
        element_count,
...
static SOC_ARM32_NEON_FORCE_INLINE float32x4_t reduce_depth_neon_arm32(
...
static SOC_ARM32_NEON_FORCE_INLINE float reduce_depth_scalar_arm32(
    float accumulated,
    float candidate
)
{
...
static void store_constant_depth_block_f32_neon_arm32(
    float* destination,
    size_t row_stride,
...
static SOC_ARM32_NEON_FORCE_INLINE float32x4_t clamp_plane_depth_neon_arm32(
    float32x4_t depth
)
{
...
static SOC_ARM32_NEON_FORCE_INLINE float clamp_plane_depth_tail_arm32(
    float depth
)
{
...
static void store_depth_plane_block_f32_neon_arm32(
    float* destination,
    size_t row_stride,
    uint32_t block_width,
    uint32_t block_height,
    ...
    for (row = 0u; row < block_height; ++row) {
        ...
        while (block_width - column >= 4u) {
            ...
            if (lane_mask != 0u) {
                ...
                candidates = clamp_plane_depth_neon_arm32(candidates);
                if (lane_mask != 0x0fu) {
                    const uint32x4_t covered = vtstq_u32(
                        vdupq_n_u32(lane_mask),
                        lane_bits
        ...
        for (; column < block_width; ++column) {
            ...
            if ((row_mask & (UINT32_C(1) << column)) != 0u) {
                const float candidate_depth = clamp_plane_depth_tail_arm32(
...
static void reduce_hiz_row_scalar_arm32(
    const float* top,
    const float* bottom,
...
)
{
    ...
    for (destination_x = destination_begin;
        ...
        if (source_x + 1u < source_width) {
            reduced = reduce_depth_scalar_arm32(
                reduced,
                top[source_x + 1u]
            );
        }
        if (bottom != NULL) {
            reduced = reduce_depth_scalar_arm32(reduced, bottom[source_x]);
            if (source_x + 1u < source_width) {
                reduced = reduce_depth_scalar_arm32(
                    reduced,
                    bottom[source_x + 1u]
...
static void reduce_hiz_level_f32_neon_arm32(
    const float* source,
    uint32_t source_width,
...
)
{
    ...
    for (destination_y = 0u;
        ...
        while (source_width - source_x >= 8u) {
            ...
            reduced = reduce_depth_neon_arm32(reduced, top_pairs.val[1]);
            reduced = reduce_depth_neon_arm32(
                reduced,
                bottom_pairs.val[0]
            );
            reduced = reduce_depth_neon_arm32(
...
static SOC_ARM32_NEON_FORCE_INLINE uint8_t compute_clip_outcode_f32_arm32(
    float32x4_t clip,
...
static const soc_kernel_table neon_kernels_arm32 = {
    ...
    .merge_depth_planes_f32 = merge_depth_planes_f32_neon_arm32,
    .store_constant_depth_block_f32 =
        store_constant_depth_block_f32_neon_arm32,
    .store_depth_plane_block_f32 =
        store_depth_plane_block_f32_neon_arm32,
    .reduce_hiz_level_f32 = reduce_hiz_level_f32_neon_arm32,
    .test_aabbs = soc_occlusion_test_aabbs_neon,
};

const soc_kernel_table* soc_kernel_table_neon(void)
{
    return &neon_kernels_arm32;
}
